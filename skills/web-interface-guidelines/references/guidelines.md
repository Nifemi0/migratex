...
- Shield verbatim content from translation. Wrap brand names, product names, code tokens, and technical identifiers with translate="no" so browser auto-translate leaves them intact.
...
- Text anti-aliasing and transforms. Scaling text can change smoothing. Prefer animating a wrapper instead of the text node. If artifacts persist set translateZ(0) or will-change: transform to promote to its own layer.
